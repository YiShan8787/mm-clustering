...
class micro_doppler_signature:
    def __init__(self, nd = 0, host_velocity = 0):
        ...
        if nd == 0:
            ...
            while not rospy.has_param("/node1_radarinterface/numLoops"):
                continue
            self.nd = rospy.get_param("/node1_radarinterface/numLoops")
    ...
    def ti_doppler_parser(self, radar):
        ...
        if radar.target_idx < 253:
            tmp = np.array([radar.target_idx, radar.range, radar.doppler_bin, radar.snr, radar.posX, radar.posY, radar.velX, radar.velY])
    ...
    def micro_doppler(self):
        ...
        for i in range(self.tmp_data.shape[1]):
            ...
            # range compensation [?,doppler_bin] = intensity*range^2
            mds_cur[tmp[0][0],int(self.tmp_data[2,i])] += self.tmp_data[3,i] * self.tmp_data[1,i]**2
            #radar.posX, radar.posY, radar.velX, radar.velY
    ...
    def main(self):
        rospy.init_node('micro_doppler_node_1')
        self.sub_ = rospy.Subscriber('/node1_radarinterface/radar_scan_1', RadarScan, self.ti_doppler_parser)
        self.pub_ = rospy.Publisher('/ti_mmwave/micro_doppler_1', MicroDoppler_m, queue_size=100)
        rospy.spin()
